...
HCE.DynamicDisplay.AppendPageSize = function (queryString) {
    ...
    return (queryString == "" ? "?" : "&") + "$top=" + recsperage;
}
...
HCE.DynamicDisplay.ProcessAdvancedSearch = function (field, pageNumber) {
    ...
    queryString = queryString + (queryString == "" ? "" : "&") + "$top=" + recsperpage;

    if (pageNumber > 1)
        queryString = queryString + (queryString == "" ? "" : "&") + "$skip=" + (pageNumber - 1) * recsperpage;
        ...
        switch (fieldType) {
            case "string":
                ...
                switch (operator) {
                    ...
                    case "startswith":
                    case "not startswith":
                        whereClause = operator + "(" + fieldName + ",'" + field1.replace(/'/g, "''") + "')";
                        break;

                    case "between":
                        whereClause = fieldName + " ge '" + field1.replace(/'/g, "''") + "' and " + fieldName + " le '" + field2.replace(/'/g, "''") + "'";
                        break;

                    default:
                        whereClause = fieldName + " " + operator + " '" + field1.replace(/'/g, "''") + "'";
                        break;
                ...
                if (fieldValue != "") {
                    switch (fieldValue) {
                        case "custom":
                            ...
                            dtIntValue1 = Date.parse(field1);
                            if (isNaN(dtIntValue1))
                                break;
                            dtValue1 = new Date(dtIntValue1);

                            switch (operator) {
                                case "between":

                                    //-- PARSE THE SECOND DATE FIELD --
                                    dtIntValue2 = Date.parse(field2);
                                    if (isNaN(dtIntValue2))
                                        break;
                                    dtValue2 = new Date(dtIntValue2);

                                    whereClause = "hcfilter=date(" + fieldName + ",between," + dtValue1.format("yyyy-mm-dd", true) + "," + dtValue2.format("yyyy-mm-dd", true) + ")";
                                    break;

                                default:
                                    whereClause = "hcfilter=date(" + fieldName + "," + operator + "," + dtValue1.format("yyyy-mm-dd", true) + ")";
                                    break;
                            }
                            break;
				   ...
				   '<div>' +
					  '<select class="records-per-page">' +
					    '<option>5</option>' +
						 ...
						 '<option>50</option>' +
					  '</select>' +
					  HCE.Resources.RecordsPerPage.toUpperCase() +
				   '</div>' +
				 '</div>';
